                     ...
                     int tracenum, int opnum)
{
    ...
    char lowTooLow = (lo < (char *)mem_heap_lo());
    char lowTooHigh = (lo > (char *)mem_heap_hi());
    char highTooLow = (hi < (char *)mem_heap_lo());
    char highTooHigh = (hi > (char *)mem_heap_hi());
    if ( lowTooLow || lowTooHigh || highTooLow || highTooHigh) {
...
static trace_t *read_trace(char *tracedir, char *filename)
{
    ...
    lol = fscanf(tracefile, "%d", &(trace->sugg_heapsize)); /* not used */
    lol = fscanf(tracefile, "%d", &(trace->num_ids));     
    lol = fscanf(tracefile, "%d", &(trace->num_ops));     
    lol = fscanf(tracefile, "%d", &(trace->weight));        /* not used */
    ...
    while (fscanf(tracefile, "%s", type) != EOF) {
        switch(type[0]) {
        case 'a':
            lol = fscanf(tracefile, "%u %u", &index, &size);
